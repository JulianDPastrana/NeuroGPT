...
def train(config: Dict = None) -> Trainer:
    ...
    assert config["training_style"] in {"CSM", "CSM_causal", "decoding"}, (
        f"{config['training_style']} is not supported."
    )

    assert config["architecture"] in {"GPT", "PretrainedGPT2"}, (
    ...
    if config["training_style"] == "decoding":
        ...
        # Get all .npz files (filter out directories like 'true_labels')
        all_files = sorted(
            [f for f in os.listdir(downstream_path) if f.endswith(".npz")]
        )
        train_files: list = []
        val_files: list = []
        test_files: list = []

        # Check if fold manifest file is provided (for 10-fold CV)
        manifest_file = config.get("fold_manifest_file")
        train_files = []
        val_files = []
        test_files_from_manifest = []
        
        if manifest_file and os.path.isfile(manifest_file):
            # Support structured manifest with TRAIN / VAL / TEST sections to avoid leakage
            ...
            with open(manifest_file, "r") as f:
                for raw_line in f:
                    ...
                    if current_section == "TRAIN":
                        ...
                        test_files_from_manifest.append(line)

            # If structured sections were provided, use them; otherwise fall back
            if train_files or val_files or test_files_from_manifest:
                # Remove duplicates across splits defensively
                def _dedup(seq):
                    seen = set()
                    out = []
                    for x in seq:
                ...
                train_files = _dedup(train_files)
                val_files = _dedup(val_files)
                test_files_from_manifest = _dedup(test_files_from_manifest)

                print(
                    "Loaded fold manifest -> "
                    f"train={len(train_files)}, val={len(val_files)}, "
                    f"test={len(test_files_from_manifest)}"
                )
                all_files = train_files + val_files + test_files_from_manifest
            else:
                # Legacy manifest (no section tags): treat all lines as candidate pool
                with open(manifest_file, "r") as f:
                    all_files = [line.strip() for line in f if line.strip()]
                print(f"Loaded {len(all_files)} files from fold manifest (legacy mode)")

        if len(all_files) == 18:
            # BCI2A dataset: use existing 9-fold CV
            train_folds, test_folds = cv_split_bci(all_files)
            train_files = train_folds[config["fold_i"]]
            val_files = test_folds[config["fold_i"]]
            test_files = val_files
        else:
            if train_files:
                # Structured manifest path: use provided splits (val/test may be empty)
                # Assign test_files from manifest if provided
                test_files = test_files_from_manifest if test_files_from_manifest else []
                
                if not val_files:
                    # If only train listed, fall back to 80/20 split on remaining files
                    remaining = [f for f in all_files if f not in train_files]
                    n_files = len(remaining)
                    n_val = max(2, n_files // 5)
                    val_files = remaining[:n_val]
                if not test_files:
                    test_files = val_files
            else:
                # Other datasets (e.g., ADHD) legacy path: use 80/20 split
                n_files = len(all_files)
                n_val = max(2, n_files // 5)  # 20% for validation, min 2
        ...
        validation_dataset = MotorImageryDataset(
            val_files,
            sample_keys=["inputs", "attention_mask", "labels"],
            chunk_len=config["chunk_len"],
            num_chunks=config["num_chunks"],
        ...
        # Optional separate test set if provided in manifest; else mirror validation
        if "test_files" in locals() and test_files is not None:
    ...
    trainer = make_trainer(
        ...
        seed=config["seed"]
        if config["set_seed"]
        else np.random.choice(range(1, 100000)),
        fp16=config["fp16"],
...
def make_model(model_config: Dict = None):
    ...
    if model_config["use_encoder"] == True:
        ...
        # For binary classification with BCE loss, use n_outputs=1
        # For multi-class, use num_decoding_classes
        encoder_n_outputs = (
            1
            if model_config["num_decoding_classes"] == 2
            else model_config["num_decoding_classes"]
        )

        # For binary classification with BCE loss, disable LogSoftmax
        # BCE expects raw logits, not log-probabilities
        use_log_softmax = model_config["num_decoding_classes"] != 2
    ...
    # Load pretrained model BEFORE switching to decoding mode
    # so that only encoder/embedder weights are loaded (not the decoding head)
...
def get_config(args: argparse.Namespace = None) -> Dict:
    ...
    if args.num_attention_heads == -1:
        assert (args.embedding_dim % 64) == 0, (
            f"embedding-dim needs be be multiple of 64 (currently: {args.embedding_dim})"
        )
...
def get_args() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=1000,
        type=int,
        help="number of validation steps to perform at evaluation time (default: 1000)",
    )
    parser.add_argument(
        "--test-steps",
        metavar="INT",
    ...
    parser.add_argument(
        ...
        default="",
        type=str,
        help="path to file listing fold-specific train+val files (for 10-fold CV)",
    )
