        ...
        for sub in subs:
            ...
            for sess in sessions:
                ...
                for mont in montages:
                    edf_files = os.listdir(
                        os.path.join(path, file, group, sub, sess, mont)
                    )
                    for edf in edf_files:
                        full_path = os.path.join(
                            path, file, group, sub, sess, mont, edf
                        )
                        filepath.append(full_path)
                        # pdb.set_trace()
                        shutil.move(
                            full_path,
                            os.path.join(path, group, sess + "_" + mont + "_" + edf),
                        )
                        # pdb.set_trace()
                # load_eeg(filepath[-1])
    return filepath
...
def get_epi_files(path, epi_csv, nonepi_csv, lower_bound=2599, upper_bound=1000000):
    ...
    if nonepi_csv is not None:
        ...
        nonepi_full_path = [
            path + "/no_epilepsy_edf/" + fn for fn in nonepi_filtered_files
        ]
...
def exclude_epi_subs(
    csv_file, epi_list, lower_bound=2599, upper_bound=1000000, files_all=None
):
    ...
    if files_all is None:
        ...
        all_files = files_all
    filtered_files = [
        f for f in all_files if not any(sub_id in f for sub_id in group_epi_subs)
    ]
    # pdb.set_trace()
    return filtered_files
...
def exclude_sz_subs(csv_file, lower_bound=2599, upper_bound=1000000, files_all=None):
    ...
    with open("sz_subs.txt", "r") as f:
        sz_subs = f.readlines()
    filtered_files = [
        f for f in all_files if not any(sub_id in f for sub_id in sz_subs)
    ]
    # pdb.set_trace()
    return filtered_files
