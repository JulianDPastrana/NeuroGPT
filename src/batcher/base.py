...
def _pad_seq_right_to_n(seq: np.ndarray, n: int, pad_value: float = 0.0) -> np.ndarray:
    if n == seq.shape[0]:
        return seq
    return np.concatenate(
        [
            seq,
            np.ones((n - seq.shape[0], *seq.shape[1:])) * pad_value,
        ],
        axis=0,
    )
...
class EEGDataset(Dataset):
    def __init__(
        ...
        if root_path == "":
            ...
            self.filenames = [
                root_path + fn for fn in filenames if os.path.isfile(root_path + fn)
            ]
            self.root_path = root_path

        # Removed noisy subject count debug print
    ...
    def preprocess_sample(
        self, sample, seq_len, labels=None
    ) -> Dict[str, torch.Tensor]:
        out = {}
        if self.do_normalization:
            sample = self.normalize(sample)

        chunks, seq_on = self.split_chunks(
            sample, self.chunk_len, self.ovlp, seq_len, self.start_samp_pnt
        )
        ...
        attention_mask = self._pad_seq_right_to_n(
            seq=attention_mask, n=seq_len, pad_value=0
        )
